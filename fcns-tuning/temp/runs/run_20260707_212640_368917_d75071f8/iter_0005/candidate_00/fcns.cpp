...
// Prefer vertices with smaller domain size (most constrained) to reduce branching
...
for (int v = 0; v < n; ++v) {
    if (!in_C[v]) continue;
    // Prefer vertices with larger domain size (more flexible) to increase chance of successful recoloring
    if (domain_size[v] > best_domain) {
        best_domain = domain_size[v];
        ...
        // Among same domain size, prefer higher degree to break more constraints
...
// Use a deterministic order: prefer the remembered color first, then the smallest available color
int remembered_color = remembered[u];
if (find(D.begin(), D.end(), remembered_color) != D.end()) {
    return remembered_color;
}
// Otherwise, pick the smallest color to encourage compactness
...
// Use DSATUR-like greedy: pick vertex with largest saturation degree (number of different colors used among neighbours)
vector<int> sat(n, 0);
vector<int> deg(n);
for (int i = 0; i < n; ++i) deg[i] = graph[i].size();
set<pair<int, int>> pq; // (-sat, -deg, index) for max sat, then max deg
for (int i = 0; i < n; ++i) {
    pq.insert({0, i});
}
vector<int> color(n, -1);
...
while (!pq.empty()) {
    ...
    vector<bool> used(max_color + 2, false);
    for (int u : graph[v]) {
    ...
    // Update saturation for neighbours
    for (int u : graph[v]) {
        if (color[u] == -1) {
            pq.erase({-sat[u], u});
            // Recompute saturation: number of distinct colors among neighbours
            vector<bool> seen(max_color + 2, false);
            int new_sat = 0;
            for (int w : graph[u]) {
                if (color[w] != -1 && !seen[color[w]]) {
                    seen[color[w]] = true;
                    ++new_sat;
                }
            }
