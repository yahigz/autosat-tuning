...
for (int v = 0; v < n; ++v) {
    ...
    int ds = domain_size[v];
    if (ds == 1) continue; // forced vertex, skip
    if (ds > best_domain || (ds == best_domain && ud > best_degree)) {
        best = v;
        best_domain = ds;
        best_degree = ud;
    }
...
for (int v = 0; v < n; ++v) {
    ...
    int ds = domain_size[v];
    int ud = uncolored_degree(v);
    if (ds > best_domain || (ds == best_domain && ud > best_udeg)) {
        best = v;
        best_domain = ds;
        best_udeg = ud;
    }
...
// Try to pick a color that minimizes conflicts with neighbors' singleton colors
int best_c = -1;
int best_conflict = numeric_limits<int>::max();
for (int c : D) {
    int conflict = 0;
    for (int to : graph[u]) {
        if (!in_C[to] && domain_size[to] == 1) {
            int sc = singleton_color(to);
            if (sc == c) ++conflict;
        }
    }
...
if (best_c != -1) return best_c;
// fallback: prefer remembered color
for (int c : D) {
    if (c == remembered[u]) return c;
}
return D[0];
    }
