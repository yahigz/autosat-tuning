"""HTTP progress server for solver tuning visualization."""
...
<head>
<meta charset="UTF-8">
<title>Solver Tuning Progress</title>
...
#iter-table th{background:#1e1e1e;color:#7cf;padding:6px 8px;text-align:left;cursor:default;border-bottom:1px solid #2a2a2a;white-space:nowrap}
...
<body>
<h1>Solver Tuning Progress</h1>
<div id="meta">Connecting...</div>
<section>
  <h2 id="primary-title">Primary metric over iterations</h2>
  <canvas id="primarychart"></canvas>
</section>
<section>
  <h2 id="secondary-title">Secondary metric over iterations</h2>
...
<section>
  ...
  <table id="iter-table">
    <thead>
      <tr>
        <th>#</th><th>Task</th><th>Primary</th><th>Secondary</th><th>Δ baseline</th>
...
function maybeShowDiff() {
  ...
  const labelA = `iter ${selA.iter} · ${selA.task||'?'} · primary=${selA.primary_value!=null?selA.primary_value.toFixed(2):'?'}`;
  const labelB = `iter ${selB.iter} · ${selB.task||'?'} · primary=${selB.primary_value!=null?selB.primary_value.toFixed(2):'?'}`;
...
function metricValue(item, key) {
  const summary = item.best_summary || item.summary || {};
  if (key === 'primary') {
    if (item.primary_value != null) return item.primary_value;
    if (summary.total_colors != null) return summary.total_colors;
    if (Array.isArray(summary.score)) return summary.score[0];
    if (item.best_par2 != null) return item.best_par2;
  } else {
    if (item.secondary_value != null) return item.secondary_value;
    if (summary.total_time != null) return summary.total_time;
    if (Array.isArray(summary.score)) return summary.score[1];
  }
  return null;
}
...
async function load() {
  ...
  const primaryLabel = labels.primary || (data.metric_mode === 'colors_time' ? 'colors' : 'PAR-2');
  const secondaryLabel = labels.secondary || (data.metric_mode === 'colors_time' ? 'time' : 'runtime');
  const baseline = data.baseline || {};
  const baselinePrimary = baseline.total_colors ?? data.baseline_par2 ?? null;
  const baselineSecondary = baseline.total_time ?? null;

  document.getElementById('meta').textContent =
    `run_id: ${data.run_id||'?'}  ·  iterations: ${iters.length}  ·  mode: ${data.metric_mode||'par2'}`;

  const chartLabels = iters.map(it=>'i'+it.iter);
  const primaryVals = iters.map(it=>metricValue(it, 'primary'));
  const secondaryVals = iters.map(it=>metricValue(it, 'secondary'));

  document.getElementById('primary-title').textContent = `${primaryLabel} over iterations`;
  document.getElementById('secondary-title').textContent = `${secondaryLabel} over iterations`;
  ...
  if (!primaryChart) {
    ...
    primaryChart = new Chart(ctx, {
      ...
      data: {
        labels: chartLabels,
        datasets: [
          { label: `Best ${primaryLabel}`, data: primaryVals, borderColor:'#7cf', backgroundColor:'rgba(119,204,255,0.08)', tension:0.3, pointRadius:4, fill:true },
          { label: 'Baseline', data: chartLabels.map(()=>baselinePrimary), borderColor:'#f77', borderDash:[6,3], pointRadius:0, fill:false },
        ]
      },
      options:{
        responsive:true,
        plugins:{legend:{labels:{color:'#aaa'}}},
        scales:{x:{ticks:{color:'#666'},grid:{color:'#1a1a1a'}},y:{ticks:{color:'#666'},grid:{color:'#1a1a1a'}}}
      }
    });
  } else {
    primaryChart.data.labels = chartLabels;
    primaryChart.data.datasets[0].data = primaryVals;
    primaryChart.data.datasets[1].data = chartLabels.map(()=>baselinePrimary);
    primaryChart.update();
  }
  ...
  if (!secondaryChart) {
    ...
    secondaryChart = new Chart(ctx, {
      ...
      data: {
        labels: chartLabels,
        datasets: [
          { label: `Best ${secondaryLabel}`, data: secondaryVals, borderColor:'#9c6', backgroundColor:'rgba(153,204,102,0.08)', tension:0.3, pointRadius:4, fill:true },
          { label: 'Baseline', data: chartLabels.map(()=>baselineSecondary), borderColor:'#f77', borderDash:[6,3], pointRadius:0, fill:false },
        ]
      },
      options:{
        responsive:true,
    ...
    secondaryChart.data.labels = chartLabels;
    secondaryChart.data.datasets[0].data = secondaryVals;
    secondaryChart.data.datasets[1].data = chartLabels.map(()=>baselineSecondary);
  ...
  for (const it of iters) {
    const primary = metricValue(it, 'primary');
    const secondary = metricValue(it, 'secondary');
    const delta = (baselinePrimary!=null && primary!=null) ? primary-baselinePrimary : null;
    const cls = delta==null?'na':delta<0?'good':'bad';
    const dStr = delta==null?'—':delta<0?delta.toFixed(2):'+'+delta.toFixed(2);
    const previewSource = it.code_preview || it.comparison_code || it.best_code || '';
    ...
    tr.innerHTML =
      ...
      `<td>${primary!=null?Number(primary).toFixed(2):'—'}</td>` +
      `<td>${secondary!=null?Number(secondary).toFixed(2):'—'}</td>` +
...
def write_progress(
    payload_or_run_id: Mapping[str, Any] | str,
    ...
    if isinstance(payload_or_run_id, Mapping):
        payload = dict(payload_or_run_id)
    else:
        payload = {"run_id": payload_or_run_id, "baseline_par2": baseline_par2, "iterations": iterations or []}
